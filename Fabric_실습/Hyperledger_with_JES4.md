...
## First Network + chaincode_Ex02_go

***First Network 구성***

![image](https://user-images.githubusercontent.com/43080040/66223512-a37dcd80-e70e-11e9-882d-9154d8fa25ed.png)

Org2개, Peer 총 4개, Orderer 1개, CA 총 2개

👉🏻`cd $HOME/HLF/fabric-samples/first-network` 

- cryptogen generate --config=./crypto-config.yaml` 명령어로 **인증서를 생성**한다.
- `configtxgen -profile TwoOrgsOrdererGenesis -channelID $SYS_CHANNEL -outputBlock ./channel-artifacts/genesis.block` 명령어로 configtxgen툴을 이용해 **orderer genesis block을 생성**한다
- `configtxgen-profile TwoOrgsChannel -outputCreateChannelTx ./channel-artifacts/channel.tx-channelID $CHANNEL_NAME` 명령어로 **채널 트랜잭션 아티팩트를 생성**한다
- `configtxgen-profile TwoOrgsChannel -outputAnchorPeersUpdate ./channel-artifacts/Org1MSPanchors.tx-channelID $CHANNEL_NAME -asOrg Org1MSP` 명령어로 Org1에 대한 **Anchor peer 아티팩트를 생성**한다.
- `configtxgen-profile TwoOrgsChannel -outputAnchorPeersUpdate ./channel-artifacts/Org2MSPanchors.tx -channelID $CHANNEL_NAME -asOrg Org2MSP` 명령어로 Org2에 대한 **Anchor peer 아티팩트를 생성**한다.

✔️시작하기 전 `./byfn.sh -m down` 명령어로 모든 docker를 내린다.

![image](https://user-images.githubusercontent.com/43080040/66311955-a402a800-e94a-11e9-8c84-4a256bf763e1.png)

👉🏻`./byfn.sh up -a -s couchdb` 명령어로 docker를 올린다.

![image](https://user-images.githubusercontent.com/43080040/66312093-e9bf7080-e94a-11e9-85c6-729aa6e1910a.png)

![image](https://user-images.githubusercontent.com/43080040/66312291-605c6e00-e94b-11e9-81c7-0e14b4be504d.png)
...
function generateChannelArtifacts() {
  ...
  ##################### configtxgen을 이용해 orderer Genesis block 생성 #####################
